...
//////////
		$("#verify").click(function() {
			...
			$.post("makeover_verify2.php", form_vals, function(data){ // ajax post data
                ...
                if(data.Captcha_Status == 'true'){ // if we passed security code check
					$("#results").hide();
                    ...
                    // Helper variable, keep the switch statement clean.
                    var heat_type = $("input[name='heat']:checked").val();
                    
                    // Processing rules based on heat type (gas, electric, other)
					switch(heat_type){
                        
                        // Gas heat
						case "gas":
							var customer_gas = $("input[name='utility_gas']:checked").val();
							if(data.Gas_Utility != customer_gas){ // if customer gas utility does not match what we know
                                // DEBUG alert("Customer Gas -> " + customer_gas + ", ETO Gas -> " + data.Gas_Utility);
								switch(customer_gas){ // Switch text in "Enter your readings" paragraph
									case "cascade":
									$("#customer_gas_utility").html("Cascade Natural Gas");
									break;
									
									case "NWN":
									$("#customer_gas_utility").html("NW Natural Gas");
									break
								}; // end switch(customer_gas)
								
								$("#meter_readings").show("slow"); // show "Enter Meter Readings
								$("#electric_readings_table").hide("slow"); // hide electrical
								$("#gas_readings_table").show("slow"); // show gas
								
							}; // endif customer gas utility doesn't match
							break; // case heat_type == "gas"
						
                        // Electic heat
						case "electric":
							var customer_electric = $("input[name='utility_electric']:checked").val();
							if(data.Electric_Utility != customer_electric){ // if customer gas utility does not match what we know
								//DEBUG alert("Customer Electric -> " + customer_electric + ", ETO Electric -> " + data.Electric_Utility);
								switch(customer_electric){ // Switch text in "Enter your readings" paragraph
									case "PGE":
									$("#customer_gas_utility").html("PGE");
                                    //$("#customer_gas_utility").html("Cascade Natural Gas");
									break;
									
									case "PAC":
									$("#customer_gas_utility").html("Pacific Power");
									break
								}; // end switch(customer_gas)
								
								$("#meter_readings").show("slow"); // show "Enter Meter Readings
								$("#electric_readings_table").show("slow"); // show electrical
								$("#gas_readings_table").hide("slow"); // hide gas
...
<div id="mainContent">
    <p><strong>Please answer the following questions to confirm that you’re eligible to enter
    ...
    <div id="hear" class="questions">
        ...
        <select id="hear" name="hear">
            ...
            <option>Better Living Show</option>
            <option>Newspaper</option>
            <option>Radio</option>
            <option>Utility bill insert</option>
        </select>
    ...
    <div id="oregon" class="questions">
        ...
        <input name="oregon" type="button" value="Yes" id="button_oregon_yes"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="oregon" type="button" value="No" id="button_oregon_no"/>
    ...
    <div id="singleFamily" class="questions">
        ...
        <input name="single" type="button" value="Yes" id="button_mf_yes"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="single" type="button" value="No" id="button_mf_no"/>
    ...
    <div id="mf_nq" class="nq">
        ...
        <p>To learn how to make your multifamily home more energy efficient, visit Energy Trust’s Web site or call 1-866-311-1822. You may qualify for cash-back incentives from Energy Trust and Oregon energy tax credits.</p>
    </div>

    <div id="primary" class="questions">
        <h3><strong>Is your single-family home your primary residence?</strong></h3>
        <input name="primary" type="button" value="Yes" id="button_primary_yes"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="primary" type="button" value="No" id="button_primary_no"/>
        ...
        <div id="oregon_np2" class="nq">
            ...
            <p>If you own your home as a rental or second home, you may be eligible for Energy Trust programs and cash incentives to make energy efficient improvements. For details, visit our Web site. </p>
        </div>
    </div>
    ...
    <p>
      <label>
        <input name="heat" type="radio" id="heat_electric" value="electric" <?php if($_POST['heat'] == 'electric'){ echo "checked=\"checked\"";}?> />
        Electricity </label>
      <!--//go to "utility_electric" -->
      (Heat Pump, Electric Furnace, Baseboard, Wall Units)<br />
      <label>
        <input type="radio" name="heat" value="gas" id="heat_gas" <?php if($_POST['heat'] == 'gas'){ echo "checked=\"checked\"";}?>/>
        Natural Gas</label>
      <!--//go to "utility_gas" -->
      (Natural Gas Furnace or Direct Vent Gas Heaters)<br />
      <label>
        <input type="radio" name="heat" value="other" id="heat_other" <?php if($_POST['heat'] == 'other'){ echo "checked=\"checked\"";}?>/>
        Other</label><!--//go to "heat_nq" -->
      (Oil, Propane, Wood)<br />    </p>
    </div>
    <div id="heat_nq" class="nq">
    <p>We’re sorry. Only homes heated with electricity or natural gas are eligible for the Home Energy Makeover Contest.
    <p>If you’re an Oregon customer of Portland General Electric (PGE) or Pacific Power living in a home heated with oil, propane, kerosene, butane or wood, you may be able to weatherize your home and receive free energy-saving light bulbs with help from the Oregon Department of Energy’s State Home Oil Weatherization (SHOW) program.</p>
    ...
    <p><a href="/residential/existinghomes/forms/LowCostRentersChecklist_SHOW.pdf" target="_blank">We also offer <u>low</u>-cost, no-cost tips to help you start saving energy       now</a></p>
    </div>
    ...
    <p>
      <label>
        <input type="radio" name="utility_gas" value="CNG" id="utility_gas_cascade" <?php if($_POST['utility_gas'] == 'CNG'){ echo "checked=\"checked\"";}?>/>
        Cascade Natural</label>
      <br />
      <label>
        <input type="radio" name="utility_gas" value="NWN" id="utility_gas_nwn" <?php if($_POST['utility_gas'] == 'NWN'){ echo "checked=\"checked\"";}?>/>
        NW Natural </label>
      <br />
      <label>
        <input type="radio" name="utility_gas" value="other" id="utility_gas_other" <?php if($_POST['utility_gas'] == 'other'){ echo "checked=\"checked\"";}?>/>
        Other</label><!--go to heat_nq -->
      </p>
    </div>
      <div id="heat_np" class="nq">
        <p>We’re sorry. Only homes heated with gas from NW Natural or Cascade Natural Gas are eligible for the Home Energy Makeover Contest.</p>
      For information on energy-saving programs available for your home, please contact your local natural gas provider. If you have questions, please call us at 1-866-368-7878.</div>
      <div id="utility_electric" class="questions">
      <h3>Which utility provides electric service to your home?</h3>
      <p>
        <label>
        <input type="radio" name="utility_electric" value="PGE" id="utility_electric_pge" <?php if($_POST['utility_electric'] == 'PGE'){ echo "checked=\"checked\"";}?>/>
        Portland General Electric</label>
        <br />
        <label>
        <input type="radio" name="utility_electric" value="PAC" id="utility_electric_pac" <?php if($_POST['utility_electric'] == 'PAC'){ echo "checked=\"checked\"";}?>/>
          Pacific Power </label>
        <br />
        <label>
        <input type="radio" name="utility_electric" value="other" id="utility_electric_other" <?php if($_POST['utility_electric'] == 'other'){ echo "checked=\"checked\"";}?>/>
          Other</label><!--go to heat_nq2 -->
        <br />
      </p>
      <div id="heat_np2" class="nq">
        <p>We’re sorry. Only homes heated with electricity from Portland General Electric or Pacific Power are eligible for the Home Energy Makeover Contest.</p>
        For information on energy-saving programs available for your home, please contact your local electricity provider. If you have questions, please call us at 1-866-368-7878.</div>
    </div>
    <div id="form">
      <table width="100%" border="1" cellpadding="10" bordercolor="#FFCC33">
        <tr>
          <td bgcolor="#FFFFCC" class="error">Please provide all the information requested in the form below. In some cases, you may be asked to enter your energy usage information for the last 12 months. This occurs if we are unable to access the data from your utility records. To provide usage information, you will need to refer to your electric and/or gas utility bills.
<span style="color: red;"><em><div id="Error_Text"><?php echo $Error_Text;?></div></em></span></td>
        ...
        <tr>
          <td><table width="600" border="0" align="center" cellpadding="3" cellspacing="0">
              <tr>
                <td width="45%"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">First
                  Name:</font><font face="Verdana, Arial, Helvetica, sans-serif"> <br />
                                                                 <input type="text" name="FirstName" value=
														 "<?php if (isset($_POST['FirstName'])){echo $_POST['FirstName'];}?>" />
                                                               </font></td
                                             >
                <td colspan="3"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Last
                  Name:</font><font face="Verdana, Arial, Helvetica, sans-serif"> <br />
              ...
              <tr>
                <td colspan="3"><p><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Street
                  Address:<br />
                  </font><font face="Verdana, Arial, Helvetica, sans-serif">
                    <input type="text" name="address" size="50" id="address" value="<?php if (isset($_POST['address'])){echo $_POST['address'];}?>" />
                  </font><br />
              ...
              <tr>
                <td width="45%"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">City:<br />
                  </font><font face="Verdana, Arial, Helvetica, sans-serif">
                  <input type="text" name="city" size="30" id="city" value=
												"<?php if (isset($_POST['city'])){echo $_POST['city'];}?>" />
                </font></td>
                <td width="27%"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">State:<br />
                  </font>
                    <input type="text" name="state" size="10" id="state" value=
												"<?php if (isset($_POST['state'])){echo $_POST['state'];}?>" />                </td>
                <td width="28%"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Zip
                  Code:<br />
                  </font>
                    <input name="ZipCode" type="text" size="10" maxlength="10" id="ZipCode" value=
												   "<?php if (isset($_POST['ZipCode'])){echo $_POST['ZipCode'];}?>" />                </td>
              </tr>
              <tr>
                <td colspan="3"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Phone
                  Number:<br />
                  </font><font face="Verdana, Arial, Helvetica, sans-serif">
                  <input type="text" name="phone" size="50" id="phone" value=
												"<?php if (isset($_POST['phone'])){echo $_POST['phone'];}?>" />
                    Example: 503-445-1234</font></td>
              </tr>
              <tr>
                <td colspan="3"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Email
                  Address:<br />
                  </font><font face="Verdana, Arial, Helvetica, sans-serif">
              ...
              <tr>
                <td colspan="3" bgcolor="#FFFFCC"><p align="center"><strong>About
                  Your Home</strong></p></td>
              </tr>
              <tr>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Year
                  
                  built:&nbsp;&nbsp; </font><font face="Verdana, Arial, Helvetica, sans-serif">
                                                                 <input type="text" name="yearBuilt" size="4" id="yearBuilt" value=
												"<?php if (isset($_POST['yearBuilt'])){echo $_POST['yearBuilt'];}?>" />
                                                               </font></td>
                <td colspan="2" valign="top"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Square
                  Feet:&nbsp;&nbsp; </font><font face="Verdana, Arial, Helvetica, sans-serif">
                                                                                <input type="text" name="squareFeet" size="4" id="squareFeet" value=
												"<?php if (isset($_POST['squareFeet'])){echo $_POST['squareFeet'];}?>" />
                                                                              </font></td>
              </tr>
              <tr>
                <td colspan="3">&nbsp;</td>
                ...
                <!-- ALU
              <tr>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Number of floors:&nbsp;&nbsp; </font><font face="Verdana, Arial, Helvetica, sans-serif">
                  <input type="text" name="floors" size="1" id="floors" value=
												"<?php if (isset($_POST['floors'])){echo $_POST['floors'];}?>" />
                
</font><br /><font face="Verdana, Arial, Helvetica, sans-serif" size="-2">NOTE: Include all floors with heated living spaces</font></td>
                <td colspan="2" valign="top"><font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Square
                  Feet:&nbsp;&nbsp; </font><font face="Verdana, Arial, Helvetica, sans-serif">
                                                                                <input type="text" name="squareFeet" size="4" id="squareFeet" value=
												"<?php if (isset($_POST['squareFeet'])){echo $_POST['squareFeet'];}?>" />
              ...
              <tr>
                <td colspan="3">
                	<div id="captcha">
                      <img id="captcha_image" src="securimage/securimage_show.php" alt="CAPTCHA Image" /><a href="#" onclick="document.getElementById('captcha_image').src = 'securimage/securimage_show.php?' + Math.random(); return false"><img id="captch_refresh" src="img/refresh.gif" border="0" alt="Reload Image"/></a>
                        <font face="Verdana, Arial, Helvetica, sans-serif" size="-1">Security Code:</font> <input type="text" name="code" size="8" />
                    </div></td>
              </tr>
              <tr>
        ...
        <tr>
          <td  align="center" bgcolor="#FFFFCC"><input type="submit" name="Submit" value="Submit" />
              <input type="reset" name="Reset" />
              <input type="button" name="verify" id="verify" value="Continue" /></td>
        </tr>
      </table>
    
    <div id="meter_readings" class="question">
      <p><strong>Please enter the amount of gas (therms) or electricity (kWh) you used in the last 12 months.</strong></p>
      
      <p>Lisa, I think this whole sections needs some work, let's talk about this. Also, I guess you want the PDF here, correct?</p>
      
      <p>We're sorry.  You indicated that your main source of heat was gas. 
        We are unable to locate a record of your gas usage. In order to qualify for the contest, please refer to your gas utility bills and provide information for the last 12 months below: </p>
      
        <table border="1" width="100%">
            <tr>
                <td><div id="electric_readings_table"><table width="398" border="1">
            <caption>
              Electric Utility Readings
            </caption>
            <tr>
              <th colspan="2" scope="col"><label>Electric Utility Account Number
                  <input type="text" name="electric_account_number" id="electric_account_number" />
              </label></th>
            </tr>
            <tr>
              <th colspan="2" scope="col"><label>Electric Utility:
            ...
            <tr>
              <th width="195" scope="col">Meter Reading Date </th>
              <th width="217" scope="col">Usage (kWh)</th>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month1" id="ele_month1" class="date-pick" value=
												"<?php if (isset($_POST['ele_month1'])){echo $_POST['ele_month1'];}?>"/></th>
              <td><input type="text" name="ele_reading1" id="ele_reading1" value=
												"<?php if (isset($_POST['ele_reading1'])){echo $_POST['ele_reading1'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month2" id="ele_month2" class="date-pick" value=
												"<?php if (isset($_POST['ele_month2'])){echo $_POST['ele_month2'];}?>"/></th>
              <td><input type="text" name="ele_reading2" id="ele_reading2" value=
												"<?php if (isset($_POST['ele_reading2'])){echo $_POST['ele_reading2'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month3" id="ele_month3" class="date-pick" value=
												"<?php if (isset($_POST['ele_month3'])){echo $_POST['ele_month3'];}?>"/></th>
              <td><input type="text" name="ele_reading3" id="ele_reading3" value=
												"<?php if (isset($_POST['ele_reading3'])){echo $_POST['ele_reading3'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month4" id="ele_month4" class="date-pick" value=
												"<?php if (isset($_POST['ele_month4'])){echo $_POST['ele_month4'];}?>"/></th>
              <td><input type="text" name="ele_reading4" id="ele_reading4" value=
												"<?php if (isset($_POST['ele_reading4'])){echo $_POST['ele_reading4'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month5" id="ele_month5" class="date-pick" value=
												"<?php if (isset($_POST['ele_month5'])){echo $_POST['ele_month5'];}?>"/></th>
              <td><input type="text" name="ele_reading5" id="ele_reading5" value=
												"<?php if (isset($_POST['ele_reading5'])){echo $_POST['ele_reading5'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month6" id="ele_month6" class="date-pick" value=
												"<?php if (isset($_POST['ele_month6'])){echo $_POST['ele_month6'];}?>"/></th>
              <td><input type="text" name="ele_reading6" id="ele_reading6" value=
												"<?php if (isset($_POST['ele_reading6'])){echo $_POST['ele_reading6'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month7" id="ele_month7" class="date-pick" value=
												"<?php if (isset($_POST['ele_month7'])){echo $_POST['ele_month7'];}?>"/></th>
              <td><input type="text" name="ele_reading7" id="ele_reading7" value=
												"<?php if (isset($_POST['ele_reading7'])){echo $_POST['ele_reading7'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month8" id="ele_month8" class="date-pick" value=
												"<?php if (isset($_POST['ele_month8'])){echo $_POST['ele_month8'];}?>"/></th>
              <td><input type="text" name="ele_reading8" id="ele_reading8" value=
												"<?php if (isset($_POST['ele_reading8'])){echo $_POST['ele_reading8'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month9" id="ele_month9" class="date-pick" value=
												"<?php if (isset($_POST['ele_month9'])){echo $_POST['ele_month9'];}?>"/></th>
              <td><input type="text" name="ele_reading9" id="ele_reading9" value=
												"<?php if (isset($_POST['ele_reading9'])){echo $_POST['ele_reading9'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month10" id="ele_month10" class="date-pick" value=
												"<?php if (isset($_POST['ele_month10'])){echo $_POST['ele_month10'];}?>"/></th>
              <td><input type="text" name="ele_reading10" id="ele_reading10" value=
												"<?php if (isset($_POST['ele_reading10'])){echo $_POST['ele_reading10'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month11" id="ele_month11" class="date-pick" value=
												"<?php if (isset($_POST['ele_month11'])){echo $_POST['ele_month11'];}?>"/></th>
              <td><input type="text" name="ele_reading11" id="ele_reading11" value=
												"<?php if (isset($_POST['ele_reading11'])){echo $_POST['ele_reading11'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="ele_month12" id="ele_month12" class="date-pick" value=
												"<?php if (isset($_POST['ele_month12'])){echo $_POST['ele_month12'];}?>"/></th>
              <td><input type="text" name="ele_reading12" id="ele_reading12" value=
												"<?php if (isset($_POST['ele_reading12'])){echo $_POST['ele_reading12'];}?>"/></td>
            </tr>
          </table></div></td>
                <td><div id="gas_readings_table"><table width="398" border="1">
            <caption>
              Gas Utility Readings
            </caption>
            <tr>
              <th colspan="2" scope="col"><label>Gas Utility Account Number
                  <input type="text" name="gas_account_number" id="gas_account_number" />
              </label></th>
            </tr>
            <tr>
              <th colspan="2" scope="col"><label>Gas Utility:
            ...
            <tr>
              <th width="194" scope="col">Meter Reading Date </th>
              <th scope="col">Usage (therms)</th>
            </tr>
            <tr>
              <th scope="row"><input name="gas_month1" type="text" class="date-pick" id="gas_month1" value=
												"<?php if (isset($_POST['gas_month1'])){echo $_POST['gas_month1'];}?>"/></th>
              <td><input type="text" name="gas_reading1" id="gas_reading1" value=
												"<?php if (isset($_POST['gas_reading1'])){echo $_POST['gas_reading1'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month2" id="gas_month2" class="date-pick" value=
												"<?php if (isset($_POST['gas_month2'])){echo $_POST['gas_month2'];}?>"/></th>
              <td><input type="text" name="gas_reading2" id="gas_reading2" value=
												"<?php if (isset($_POST['gas_reading2'])){echo $_POST['gas_reading2'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month3" id="gas_month3" class="date-pick" value=
												"<?php if (isset($_POST['gas_month3'])){echo $_POST['gas_month3'];}?>"/></th>
              <td><input type="text" name="gas_reading3" id="gas_reading3" value=
												"<?php if (isset($_POST['gas_reading3'])){echo $_POST['gas_reading3'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month4" id="gas_month4" class="date-pick" value=
												"<?php if (isset($_POST['gas_month4'])){echo $_POST['gas_month4'];}?>"/></th>
              <td><input type="text" name="gas_reading4" id="gas_reading4" value=
												"<?php if (isset($_POST['gas_reading4'])){echo $_POST['gas_reading4'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month5" id="gas_month5" class="date-pick" value=
												"<?php if (isset($_POST['gas_month5'])){echo $_POST['gas_month5'];}?>"/></th>
              <td><input type="text" name="gas_reading5" id="gas_reading5" value=
												"<?php if (isset($_POST['gas_reading5'])){echo $_POST['gas_reading5'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month6" id="gas_month6" class="date-pick" value=
												"<?php if (isset($_POST['gas_month6'])){echo $_POST['gas_month6'];}?>"/></th>
              <td><input type="text" name="gas_reading6" id="gas_reading6" value=
												"<?php if (isset($_POST['gas_reading6'])){echo $_POST['gas_reading6'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month7" id="gas_month7" class="date-pick" value=
												"<?php if (isset($_POST['gas_month7'])){echo $_POST['gas_month7'];}?>"/></th>
              <td><input type="text" name="gas_reading7" id="gas_reading7" value=
												"<?php if (isset($_POST['gas_reading7'])){echo $_POST['gas_reading7'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month8" id="gas_month8" class="date-pick" value=
												"<?php if (isset($_POST['gas_month8'])){echo $_POST['gas_month8'];}?>"/></th>
              <td><input type="text" name="gas_reading8" id="gas_reading8" value=
												"<?php if (isset($_POST['gas_reading8'])){echo $_POST['gas_reading8'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month9" id="gas_month9" class="date-pick" value=
												"<?php if (isset($_POST['gas_month9'])){echo $_POST['gas_month9'];}?>"/></th>
              <td><input type="text" name="gas_reading9" id="gas_reading9" value=
												"<?php if (isset($_POST['gas_reading9'])){echo $_POST['gas_reading9'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month10" id="gas_month10" class="date-pick" value=
												"<?php if (isset($_POST['gas_month10'])){echo $_POST['gas_month10'];}?>"/></th>
              <td><input type="text" name="gas_reading10" id="gas_reading10" value=
												"<?php if (isset($_POST['gas_reading10'])){echo $_POST['gas_reading10'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month11" id="gas_month11" class="date-pick" value=
												"<?php if (isset($_POST['gas_month11'])){echo $_POST['gas_month11'];}?>"/></th>
              <td><input type="text" name="gas_reading11" id="gas_reading11" value=
												"<?php if (isset($_POST['gas_reading11'])){echo $_POST['gas_reading11'];}?>"/></td>
            </tr>
            <tr>
              <th scope="row"><input type="text" name="gas_month12" id="gas_month12" class="date-pick" value=
												"<?php if (isset($_POST['gas_month12'])){echo $_POST['gas_month12'];}?>"/></th>
              <td><input type="text" name="gas_reading12" id="gas_reading12" value=
												"<?php if (isset($_POST['gas_reading12'])){echo $_POST['gas_reading12'];}?>"/></td>
            </tr>
          </table></div></td>
            </tr>
        </table>
      ...
      <input type="submit" name="enter_readings" id="enter_gas" value="Barney"/>
    </form>
    <?php 
	if($debug){
		echo "<pre>";
